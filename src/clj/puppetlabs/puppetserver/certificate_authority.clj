...
(def MasterSettings
  "Settings from OpenVox that are necessary for SSL initialization on the master.
   ...
   All of these are OpenVox configuration settings."
  {:certdir        schema/Str
...
(def CaSettings
  "Settings from OpenVox that are necessary for CA initialization
   and request handling during normal OpenVox operation.
   Most of these are OpenVox configuration settings."
...
(schema/defn next-serial-number! :- schema/Int
  ...
  OpenVox's $serial setting defines the location of the serial number file."
...
(schema/defn ^:always-validate
  ...
  "Writes an entry into OpenVox's inventory file for a given certificate.
  The location of this file is defined by OpenVox's 'cert_inventory' setting.
          ...
          (if-let [infra-node (first local-infra-nodes)]
            (let [augmented-results (concat result (find-matching-valid-serial-numbers settings infra-node))
              ...
              (if (fs/exists? cert-path)
                ...
                (do
                  (log/warn (i18n/trs "Failed to find certificate for OpenVox Infrastructure Node: {0}" infra-node))
                  (recur (rest local-infra-nodes)
                         augmented-results))))
             ...
             (when-let [write-exception (write-local-cacrl!
               ...
               (if (zero? attempts-left)
                 (log/error (format "%s\n%s\n%s"
                                    ...
                                    (i18n/trs
                                      "To load the updated CRL, reload or restart the OpenVox Server service.")))
                 (do
                   (Thread/sleep 100)
...
(schema/defn execute-autosign-command!
  ...
  "Execute the autosign script and return a map containing the standard-out,
   ...
   the OpenVox and OpenFact Ruby libraries available to the autosign script.
...
(schema/defn ^:always-validate
  ...
  "Given the configuration map from the OpenVox Server config
...
(schema/defn ^:always-validate
  ...
  OpenVox's autosign setting, and false otherwise."
  ([autosign :- AutoSignInput
    subject :- schema/Str
    csr-stream :- InputStream]
...
(schema/defn ^:always-validate
  ...
  from OpenVox, auto-sign the request and write the certificate to disk."
  [subject :- schema/Str
   csr :- CertificateRequest
...
(schema/defn ^:always-validate
  ...
  "Given the value of the 'cacrl' setting from OpenVox,
...
(schema/defn ^:always-validate maybe-replace-crl :- CertificateRevocationList
  ...
  is the given CRL. Never replaces the CRL corresponding to the OpenVox
  CA signing cert."
  [crl :- CertificateRevocationList
...
(schema/defn filter-already-revoked-serials :- [BigInteger]
  "Given a list of serials and OpenVox's CA CRL, returns vector of serials with
