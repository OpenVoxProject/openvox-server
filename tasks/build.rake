...
namespace :vox do
  ...
  task :build, [:tag] do |_, args|
    begin
      ...
      run("cd /code && COW=\"#{@debs}\" MOCK=\"#{@rpms}\" GEM_SOURCE='https://rubygems.org' EZBAKE_ALLOW_UNREPRODUCIBLE_BUILDS=true EZBAKE_NODEPLOY=true LEIN_PROFILES=ezbake EZ_VERBOSE=true lein with-profile user,ezbake,provided,internal ezbake local-build")
